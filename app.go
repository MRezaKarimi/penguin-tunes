...
// ReadIndex returns the raw index.json file contents as a string.
// If the index file does not exist, it returns an empty string and no error.
func (a *App) ReadIndex() (string, error) {
	if a.idxPath == "" {
		return "", nil
	}
	b, err := os.ReadFile(a.idxPath)
	if err != nil {
